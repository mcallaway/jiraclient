...
sub test_bad_opt {
  my $obj = test_start();
  my $opts = "-n -d -q $cwd/data/sample-fasta/sample-fasta-1";
  my $res;
...
sub test_debug_dryrun_opts {
  my $obj = test_start();
  my $opts = "-n -d $cwd/data/sample-fasta/sample-fasta-1";
  throws_ok { $obj->main(ostr($opts)); } qr/no action specified/, "no action caught ok";
  $opts = "-n -s -d $cwd/data/sample-fasta/sample-fasta-1";
...
sub test_set_cache {
  my $obj = test_start();
  my $opts = "-i $cwd/data/test.cache -v -d $cwd/data/sample-fasta/sample-fasta-1";
  ok($obj->main(ostr($opts)) == 0,"test run went ok");
...
sub test_set_logfile {
  my $obj = test_start();
  my $opts = "-l $cwd/testlog -v -d $cwd/data/sample-fasta/sample-fasta-1";
  ok($obj->main(ostr($opts)) == 0,"test run went ok");
