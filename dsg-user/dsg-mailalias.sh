...
Options:
  -a,--alldsg            subscribe user to allgsc mailing list
  -f,--forward=ADDR   forward all email to account to ADDR
  -h,--help           print this message and exit
  -v,--version        print version number and exit

This is meant to be called from dsg-useradd to create the user home
directory on the mail server and subscribe user to dsg.
...
# subscribe user to dsg mailing list
if [ "$dsg" -eq 1 ]; then
    email=$login@dsgmail.wustl.edu
    subscribe="add_members -r - dsg"
