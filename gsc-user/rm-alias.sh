...
# remove from specific alias files
for file in /etc/postfix/lists/*; do
    # skip special files
    case "$file" in
        aliases | external | *.db | *.bak)
            continue
            ;;
    esac
    ...
    if cp "$file" "$file.bak"; then
        ...
        echo "$pkg: failed to make backup of $file"
        status=1
        continue
    fi

    # remove occurrence of user from file
    if sed "/^$login\$/d" "$file.bak" >"$file"; then
        :
    else
        echo "$pkg: editing of $file failed"
        status=1
    fi
done    

# see if login is in aliases file
alias=/etc/postfix/lists/aliases
